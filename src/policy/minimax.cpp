 ...
 * @return Move
 */
Move MiniMax::get_move(T_Node* root){
...
bool MiniMax::build_tree(int depth, int player, State *state, T_Node *root){
    root->depth  = depth;
    root->id     = ++node_cnt;
    root->player = player;
    root->state  = state;
...
int MiniMax::minimax_tree(T_Node *root, int depth, int maximizingPlayer){
    ...
    if (maximizingPlayer){
        best_value = INT_MIN;
        for (auto iter = root->child_vec.begin(); iter != root->child_vec.end(); ++iter)
            best_value = max(best_value, minimax_tree(*iter, depth+1, 0));
        root->value = best_value;
        return best_value;
    }else{ /* minimizing player */
        best_value = INT_MAX;
        for (auto iter = root->child_vec.begin(); iter != root->child_vec.end(); ++iter)
            best_value = min(best_value, minimax_tree(*iter, depth+1, 1));
        root->value = best_value;
        return best_value;
    }
}

void MiniMax::node_output(T_Node *root){
...
void MiniMax::DFS_output(T_Node *root){
    node_output(root);
    if (root->depth < MAX_DEPTH){
        for (auto iter = root->child_vec.begin(); iter != root->child_vec.end(); ++iter)
            DFS_output(*iter);
...
void MiniMax::BFS_output(T_Node *t_root){
    queue<T_Node *> q;
    q.push(t_root);
    T_Node *root = nullptr;
    while(!q.empty()){
